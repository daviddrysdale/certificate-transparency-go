...
type ScannerOptions struct { // nolint:golint
	...
	// Certificate found during scanning.  Should be a Matcher or RawMatcher
	// implementation.
	Matcher interface{}
...
func (s *Scanner) processEntry(index int64, entry ct.LeafEntry, foundCert func(*ct.LogEntry), foundPrecert func(*ct.LogEntry)) error {
	...
	case RawMatcher:
		return s.processRawMatcherEntry(matcher, index, entry, foundCert, foundPrecert)
	default:
...
func (s *Scanner) processRawMatcherEntry(matcher RawMatcher, index int64, entry ct.LeafEntry, foundCert func(*ct.LogEntry), foundPrecert func(*ct.LogEntry)) error {
	if matcher.LeafMatches(&entry) {
		logEntry, err := ct.LogEntryFromLeaf(index, &entry)
		if logEntry == nil {
