...
func TestLaxPropagate(t *testing.T) {
	b := []byte{0x30, 0x06, // SEQUENCE length 6
		...
		'c', 'a', 'f', 0xE9}
	var result stringHolder

	expectedErr := "invalid character"
	_, err := UnmarshalWithParams(b, &result, "")
	if err == nil {
		t.Errorf("Unmarshal succeeded")
	} else if !strings.Contains(err.Error(), expectedErr) {
		t.Fatalf("Expected error to mention %q but error was %q", expectedErr, err.Error())
	}

	_, err = UnmarshalWithParams(b, &result, "lax")
	if err != nil {
		t.Errorf("Unmarshal failed in lax mode")
	}
	if result.Value != "café" {
